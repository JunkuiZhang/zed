...
pub struct LineWrapper {
    ...
    cached_other_grapheme_widths: HashMap<String, Pixels>,
}
...
impl LineWrapper {
    ...
    pub(crate) fn new(
        ...
        Self {
            ...
            cached_other_grapheme_widths: HashMap::default(),
        }
    }
    ...
    fn width_for_char(&mut self, c: &str) -> Pixels {
        ...
        } else if let Some(cached_width) = self.cached_other_grapheme_widths.get(c) {
            *cached_width
        } else {
            let width = self.compute_width_for_grapheme(c);
            self.cached_other_grapheme_widths
                .insert(c.to_owned(), width);
            width
        }
    }
